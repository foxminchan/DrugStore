    ...
    : IQueryHandler<GetByUserId, Result<BasketVm>>
{
    public Task<Result<BasketVm>> Handle(GetByUserId request, CancellationToken cancellationToken)
    {
        ...
        var basket = redisService.HashGetOrSet<CustomerBasket>(key, request.CustomerId.ToString(), () => new());
        ...
        var pagedResult = new PagedResult<List<BasketItem>>(pagedInfo, items);
